...
import {
  Box, Card, CardContent, Typography, TextField, IconButton, Tooltip,
  Table, TableHead, TableRow, TableCell, TableBody, Chip, Button, Dialog,
    ...
    <Box sx={{ display:'flex', minHeight:'100dvh', bgcolor:'#F3F7FF' }}>
      ...
      <Box component="main" sx={{ flex:1, p:3, display:'grid', alignContent:'start' }}>
        ...
        <Card sx={{
          ...
          <CardContent sx={{ p:2, height:'100%', display:'grid', gridTemplateRows:'auto 1fr' }}>
            {/* Search row */}
              ...
              <Table size="small" stickyHeader>
                ...
                <TableBody>
                  {filtered.map(u => {
                    ...
                    return (
                      <TableRow key={u.email} hover>
                        ...
                        <TableCell>
                          <Chip size="small" label={u.is_active ? 'Yes' : 'No'}
                        ...
                        <TableCell align="right">
                          <Tooltip title="Change password">
                            <IconButton onClick={()=>setPwdDlg({ open:true, email:u.email })} size="small">
                              <KeyIcon fontSize="small" />
                            </IconButton>
                          </Tooltip>
                          <Tooltip title="Delete user">
                            <IconButton onClick={()=>setConfirm({ open:true, email:u.email })} size="small" sx={{ ml: 0.5 }}>
                  ...
                  {(!busy && filtered.length === 0) && (
                    <TableRow><TableCell colSpan={8} align="center">No users</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </Box>
      ...
      {/* Confirm delete */}
      <Dialog open={confirm.open} onClose={()=>setConfirm({ open:false, email:null })}>
        <DialogTitle>Delete user?</DialogTitle>
        <DialogContent>Are you sure you want to delete <b>{confirm.email}</b>?</DialogContent>
        <DialogActions>
